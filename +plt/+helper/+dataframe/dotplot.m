function dotplot(group,y,varargin)
...
%       in general: 'formating_optionl', value,...
...
while(c<=length(varargin))
    switch(varargin{c})
    case {'gapwidth','linscale','split','subset','leg','leglocation','dir','sizedata'}
...
%   F.fillcolor
        ...
        fm.tag         = num2str(c);          
        [dummy,h(c)]   = plt.helper.dataframe.mydotutil(D{c,2}(~isnan(D{c,2})),x_coord(c),fm);
    end;
end;
